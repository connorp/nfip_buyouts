...
```{r first-stage-graph, echo=FALSE}
...
```{r trans-prob, echo=FALSE}
transtime <- tzy_panel[in_sample == TRUE, .(transfrac = sum(transaction_count) / sum(properties_count)), keyby = .(year, adapted)]
ggplot(transtime, aes(x=year, y=transfrac)) + geom_point(aes(color = adapted)) + geom_vline(xintercept=4.5) + geom_vline(xintercept=6.5) + ggtitle("Home Sale Probability Over Time") + ylab("Probability of Home Sale") + geom_text(aes(x=4.25, label="Biggert Waters Act", y=.055), angle=90)  + geom_text(aes(x=6.25, label="HFIAA", y=.055), angle=90)
```
...
```{r transprob-byflood, echo=FALSE}
...
stargazer(transprob_reg_2, transprob_reg_3, transprob_reg_2_lead, transprob_reg_3_lead, 
          ...
          dep.var.caption = "Probability of Home Sale", dep.var.labels.include = FALSE, 
          column.sep.width = "2pt", df=FALSE, digits = 4, header = FALSE, keep.stat = "n")
```
